...
nxtpage = async()=>{
  ...
  render() {
    ...
    return (
      ...
      <h2 className='title'>Top Headlines-{this.props.category}</h2>
      <div className='row'>
        {this.state.articles.map((element)=>{
         return <div className='col-md-3 my-3' key={element.url}>
          <Newsitem  title={element.title}  description={element.description}  imageurl={element.urlToImage} url={element.url} source={element.source.name}/>
          </div>
  })}
